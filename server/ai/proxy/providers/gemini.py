...
class GeminiProvider(AIProvider):
    ...
    def _get_client_and_model(self, api_key: Optional[str], model: Optional[str]):
        """
        Helper to determine which client and model to use.
        If a custom api_key is provided, we create a temporary client.
        Otherwise we use the default client.
        """
        ...
        if api_key and api_key != self.default_api_key:
            ...
            return genai.Client(api_key=api_key), target_model
        else:
            # Use the persistent default client
    ...
    def chat(self, history: List[Dict[str, Any]], new_message: str) -> str:
        ...
        """
        # Chat usually relies on a persistent session, so dynamic config here is tricky without recreating session.
        # For now, we use the default client/model.
        try:
