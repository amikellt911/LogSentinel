...
引入学术界 LogBatcher 的核心思想，在日志进入 AI 分析流程前增加一个“解析/模版提取”层，以实现降本增效和数据结构化。

-   模版分离与结构化:

    -   目标：将非结构化的原始日志（Raw Log）转化为结构化的数据。

    -   实现：利用聚类算法或正则匹配，将日志分离为 模版 (Template) 和 参数 (Variables)。例如，将 User Alice login failed 和 User Bob login failed 统一提取为模版 User <*> login failed。

-   缓存优先机制 (Caching First):

    -   目标：大幅降低 LLM 调用频率和 Token 消耗。

    -   实现：构建本地缓存（如 SQLite 或内存 Map），存储已识别的模版正则。新日志到达时先查缓存，匹配成功则直接提取参数，无需调用 LLM。只有缓存未命中的新日志模式才透传给 AI 进行处理。

-   微批处理 (Micro-batching):

    -   目标：提高 AI 识别模版的准确率和效率。

    -   实现：对于未命中的日志，采用微批处理策略，将一批具有多样性的日志打包发送给 LLM 进行模版学习，而不是单条处理。
