...
int main()
{
    ...
    auto onRequest = [&tpool, persistence](HttpRequest &req, HttpResponse *resp, const MiniMuduo::net::TcpConnectionPtr &conn)
    {
        if (req.method() == "POST" && req.path() == "/logs")
        {
            ...
            auto work = [task, persistence]
            {
                try
                {
                    ...
                    // 步骤 2: 调用 Mock AI
                    std::string ai_result = mock_ai_analysis(task.raw_request_body);
