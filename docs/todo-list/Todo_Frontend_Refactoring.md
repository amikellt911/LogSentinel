...
**目标**：为了答辩演示只暴露已经跑通的主链页面，先把半成品入口从前台收掉，但不直接删除页面文件。

- [x] 从侧边栏移除 `Live Logs` 导航入口
- [x] 从侧边栏移除 `Performance Arena` 导航入口
- [x] 将旧 `/logs` 路由重定向到稳定页面，避免历史书签落到半成品
- [x] 将旧 `/benchmark` 路由重定向到稳定页面，避免历史书签落到半成品
- [x] 追加本次前端收口 dev-log 记录
