...
- benchmark 材料当前已经不是“从 0 开始”：
  ...
  - `Suite B` 的 `run_suite_b_matrix.py` 已支持直接用 CLI 指定后端绑核、I/O 线程和工作线程资源，不再强依赖手写 `server-command` 模板。
  - `Suite B` 的矩阵 runner 已补端口占用预检，避免旧进程占着 case 端口时把 readiness 误判成新后端已启动。
